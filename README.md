...
### For the tutor case:
 ...
 2. From the dashboard (or profile settings page in the signup case), the tutor can navigate to the Profile Settings page (accessed via the top right corner). Here, they can update their personal information and add details for any upcoming sessions by clicking on edit and then saving.
...
### For the student case:
 ...
 4. The student has two options:
    - View the tutor’s profile for additional details about qualifications, reviews, and availability.
    - Book a session with the recommended tutor. For booking, the student inputs the desired date and time, and then proceeds to the payment stage.

 6. Upon successful payment, the student can navigate to the “My Sessions” tab, where all upcoming sessions are listed. At the designated time, the student clicks the “Join” button to enter the session.

 7. After the session concludes, the student is prompted to provide feedback and leave a review. This input not only helps the tutor improve but also refines future matching recommendations for the student.
